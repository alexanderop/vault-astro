...
## [2026-04-20] ingest | The Friction is Your Judgment (Armin Ronacher & Cristina Poncela Cubeiro, AI Engineer 2026)

- Source captured: `sources/friction-is-your-judgment.md` (full transcript from YouTube, speakers' verbatim words cleaned of auto-transcript mis-hearings like "Arendelle" → "Earendil").
- Wiki note: [[friction-is-your-judgment]] — Core thesis: friction is the steering mechanism and "ship without friction" is the failure mode. Two problems converge: psychology (the efficiency illusion — output volume ≠ value; baseline shifts fast) and engineering (agents optimize for "make progress" not "good code," creating brittle fallbacks and entropy faster than humans would). Libraries thrive under agents because constraints are the spec; products suffer because UI × API × permissions × billing can't fit a context window. The agent-legible codebase answer: lint out hidden magic (no bare catches, one SQL interface, no dynamic imports, unique function names, erasable-syntax TypeScript). The Pi review split — mechanical issues go back to the agent, judgment calls (DB migrations, permission changes, new dependencies, architecture) wake the human — is the actionable insight. SLO analogy: friction was always intentional; agent tooling now promises to remove it exactly where you need it.
- New author profile: `authors/cristina-poncela-cubeiro.md` (Earendil engineer, ex-Bending Spoons, self-described native AI engineer).
- Diagram: flowchart of the productivity-trap cycle with the judgment-call break-out — where human review must re-enter the loop (migrations, permissions, architecture).
- Connections: [[pi-coding-agent-the-best-alternative-to-claude-code]] (Pi is Ronacher's review tool — first product-side argument for _why_ the harness must be customizable), [[dhhs-new-way-of-writing-code]] (DHH's "taste is the bottleneck" + this talk's "judgment needs friction" are two sides of the same shift), [[the-wet-codebase]] (inline-first and agent-legible share the simple-flat-obvious root), [[biggest-problems-of-ai]] (productivity illusion is a first-order safety problem, not just taste).

## [2026-04-20] ingest | A Philosophy of Software Design — Ousterhout (Talks at Google)

- Source captured: `sources/a-philosophy-of-software-design-talks-at-google.md` (YouTube, full transcript).
- Wiki note: [[a-philosophy-of-software-design-talks-at-google]] — The origin-point Google talk for the book. Sharper than the later restatements because the framings aren't yet defending against counter-arguments. Three things show up cleaner here than elsewhere: the **rectangle model** for deep/shallow modules (top edge = interface cost, area = functionality; eyeball whether the abstraction earns its interface), the **tactical tornado** as a named anti-pattern (prolific producer of 80%-working shoddy code, rewarded by management as a hero), and the **hire for slope, not y-intercept** aphorism. Also the strongest statement of "define errors out of existence" — with the Tcl `unset` self-admitted mistake, Unix unlink-while-open, and Java `substring` as the three concrete examples. The chapter gets misread as "ignore errors"; it's actually "redesign the semantics so there is no error."
- No new author profile (john-ousterhout already exists).
- Diagram: two mermaid flowcharts — the rectangle model (deep vs shallow module) and the tactical vs strategic velocity curves with the 6–12 month crossover.
- Connections: [[the-philosophy-of-software-design-with-john-ousterhout]] (same speaker eight years later, AI frame added), [[ousterhout-martin-software-philosophies]] (where these positions get tested against Martin), [[deep-and-shallow-modules]] (Zdražil's book-chapter version of the rectangle model with a TypeScript example), [[the-wet-codebase]] (anti-DRY as the same shape as anti-classitis), [[tidy-first]] (Beck's "when" to invest vs Ousterhout's "how much").

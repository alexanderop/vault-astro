...
1. **Try exact slug match first**: Check if `src/content/notes/notes/{slug}.md` exists (convert spaces to hyphens, lowercase)
2. **If no exact match**: Use Grep to search for the title in book notes:
   ```text
   Grep pattern: "title:.*{search term}" with glob: "src/content/notes/notes/*.md"
   ```
...
```markdown
✓ Enhanced: src/content/notes/notes/{slug}.md

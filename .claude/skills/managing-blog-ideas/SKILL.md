...
All blog ideas live in `src/content/notes/blog-ideas/` and are excluded from the content collection.

## Mode Detection

1. **List mode**: User asks "what blog ideas" or "list blog ideas" → list all files in `src/content/notes/blog-ideas/`
...
2. Search Second Brain for related notes:
   ...
   ```text
   Grep pattern: "{topic keywords}" glob: "src/content/notes/notes/*.md"
   ```
...
Save to `src/content/notes/blog-ideas/{slug}.md`.
...
1. Find the file:
   ```text
   Glob: src/content/notes/blog-ideas/{slug}*.md
   ```
...
**Wiki-link check:** Each `[[link]]` should exist in `src/content/notes/`.

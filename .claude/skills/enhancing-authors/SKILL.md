...
1. **Try exact slug match first**: Check if `src/content/notes/authors/{slug}.md` exists (convert spaces to hyphens, lowercase)
2. **If no exact match**: Use Grep to search for the name in author files:
   ```text
   Grep pattern: "name:.*{search term}" with glob: "src/content/notes/authors/*.md"
   ```
...
```markdown
Updated: src/content/notes/authors/{slug}.md
...
| Error                | Recovery                                               |
| -------------------- | ------------------------------------------------------ |
| Author not found     | List available authors in `src/content/notes/authors/` |
...
- [ ] Author file exists in `src/content/notes/authors/`
